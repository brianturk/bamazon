        ...
        if (results[0]) {
            ...
            if (stockQuantity < quantity) {
                ...
                if (results.protocol41) {
                    resolve(`Purchase made!  Your total cost was ${formatter.format(totalCost)}`)
                } else {
...
async function itemList() {
    return new Promise(async function (resolve, reject) {
        ...
        var results = await getSQL('SELECT item_id, product_name, department_name, price FROM products WHERE stock_quantity > 0')
